...
var tags = fs.readFileSync('tags.txt', 'utf-8');
...
//fuction to check some tags are present or not
function checking_tags() {

	var token_mydocument = tokenizer.tokenize(standard_document);
	var token_tags = tokenizer.tokenize(tags);
	var spellcheck = new natural.Spellcheck(token_mydocument);

	for (i in token_tags) 
		if (spellcheck.isCorrect(token_tags[i].toLowerCase())) {
			extra_marks_given++;
			concept_covered.push(token_tags[i]);
		}
	}
